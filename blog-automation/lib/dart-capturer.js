...
const { getCorpCode } = require("./dartCollector");
...
async function selectDisclosures(stockCode) {
  ...
  const url = `${DART_BASE_URL}/list.json?crtfc_key=${DART_API_KEY}&corp_code=${corpCode}&bgn_de=${fmt(start)}&end_de=${fmt(end)}&page_count=40`;
...
/**
 * 콘텐츠 프레임 탐색 — table 수가 가장 많은 frame
 */
...
async function captureAnnualReport(page, stockCode, outputDir) {
  ...
  for (const section of sections) {
    try {
      ...
      const filePath = path.join(outputDir, `${stockCode}_${section.suffix}.png`);
...
async function captureGeneralDisclosure(page, stockCode, disc, outputDir) {
  try {
    ...
    const filePath = path.join(outputDir, `${stockCode}_${suffix}.png`);
    const ok = await screenshotLargestTable(contentFrame, filePath);
 ...
 * @param {function} progressCb - (idx, total, desc) 진행 콜백
 * @returns {Promise<string[]>} 저장된 파일 경로 배열
 */
async function captureDartPages(disclosures, stockCode, outputDir, progressCb) {
  if (!disclosures || disclosures.length === 0) return [];

  const notify = (idx, total, desc) => {
    console.log(`  [DART ${idx}/${total}] ${desc}`);
    if (progressCb) progressCb(idx, total, desc);
  };
  ...
  try {
    ...
    for (let i = 0; i < disclosures.length; i++) {
      ...
      notify(i + 1, disclosures.length, `${disc.title} 캡처...`);
